...
- `cli/Cargo.toml` - Dependencies file (added arangors = "0.6.0" and tokio runtime for async operations)

### Notes

- ArangoDB dependency (`arangors = "0.6.0"`) is already present in `cli/Cargo.toml`
- Added tokio runtime with features ["rt", "rt-multi-thread"] to support async ArangoDB operations
...
- [x] 1.0 Setup ArangoDB connection and implement title retrieval
  - [x] 1.1 Create `connect_to_arangodb()` function in `cli/src/bootstrap/suttacentral.rs`
  ...
  - [x] 1.2 Implement `get_titles()` function to query 'names' collection
  ...
  - [x] 1.3 Add unit test for ArangoDB connection
  ...
  - [x] 1.4 Add unit test for title retrieval
...
- [x] 2.0 Implement helper functions for UID generation and filtering
  - [x] 2.1 Implement `html_text_uid()` function
  ...
  - [x] 2.2 Implement `bilara_text_uid()` function
  ...
  - [x] 2.3 Implement `res_is_ignored()` function
    - Accept parameter: `doc: &Document`
  ...
  - [x] 2.4 Implement `uid_is_ignored()` function
    ...
    - Return `false` otherwise
  - [x] 2.5 Implement `convert_paths_to_content()` function
    ...
    - Return `Result<()>`
  - [x] 2.6 Add unit tests for UID generation functions
  ...
  - [x] 2.7 Add unit tests for filtering functions
...
- [x] 3.0 Implement Bilara template collection
  - [x] 3.1 Implement `get_bilara_templates()` function
    ...
    - Return `Result<HashMap<String, String>>`
  - [x] 3.2 Add unit test for template collection
...
- [x] 4.0 Implement sutta retrieval from ArangoDB (both html_text and sc_bilara_texts)
  - [x] 4.1 Implement `html_text_to_sutta()` function
  ...
  - [x] 4.2 Implement `bilara_text_to_sutta()` function
    ...
    - Build and return `SuttaData` struct
  - [x] 4.3 Implement `get_suttas()` function - main query logic
  ...
  - [x] 4.4 Implement deduplication logic within `get_suttas()`
  ...
  - [x] 4.5 Add unit tests for sutta conversion functions
...
- [x] 5.0 Implement sutta variants and comments import
  - [x] 5.1 Implement `import_sutta_variants()` function
    ...
    - Return `Result<()>`
  - [x] 5.2 Implement `import_sutta_comments()` function
    ...
    - Return `Result<()>`
  - [x] 5.3 Add unit tests for variants and comments
...
- [x] 6.0 Integrate SuttaCentralImporter with bootstrap process
  - [x] 6.1 Create `SuttaCentralImporter` struct
    - Add to `cli/src/bootstrap/suttacentral.rs`
    - Fields: `sc_data_dir: PathBuf`
    - Implement `new(sc_data_dir: PathBuf) -> Self`
  - [x] 6.2 Implement `SuttaImporter` trait for `SuttaCentralImporter`
  ...
  - [x] 6.3 Implement `import_for_language()` private method
    ...
    - Log: "DONE: {lang}"
    - Return `Result<()>`
  - [x] 6.4 Update `cli/src/bootstrap/mod.rs` to use SuttaCentralImporter
  ...
  - [x] 6.5 Verify bootstrap process compiles and runs
...
- [x] 7.0 Testing and validation
  - [x] 7.1 Test with small dataset (BOOTSTRAP_LIMIT=10)
  ...
  - [x] 7.6 Document any known issues or limitations
...
### Completed Features
- ✅ ArangoDB connection and queries
- ✅ Title retrieval for both Pāli and English
- ✅ Bilara HTML template collection (7379 templates loaded)
- ✅ UID generation for both html_text and bilara_text formats
- ✅ Filtering logic to skip site pages, playgrounds, etc.
- ✅ Sutta retrieval from both html_text and sc_bilara_texts collections
- ✅ Deduplication logic (prefer Bilara root, skip references/variants)
- ✅ Text processing (niggahita, ASCII, compact_rich_text)
- ✅ HTML extraction and Bilara JSON to HTML conversion
- ✅ Sutta variants import with foreign key linking
- ✅ Sutta comments import with foreign key linking
- ✅ SuttaCentralImporter struct implementing SuttaImporter trait
- ✅ Integration with bootstrap process in mod.rs
- ✅ Unit tests for all major functions
- ✅ Error handling with graceful degradation

### Tests Passed
All unit tests pass including ignored tests that require ArangoDB:
- `test_connect_to_arangodb` - Successfully connects to ArangoDB
- `test_get_titles_english` - Retrieved 19,251 English titles
- `test_get_titles_pali` - Retrieved 24,223 Pāli titles (including DN1, MN1)
- `test_get_bilara_templates` - Retrieved 7,379 HTML templates
- All UID generation tests pass
- All filtering tests pass
- Sutta conversion tests pass

Run with: `cd cli && cargo test -- --ignored --nocapture`

### Known Limitations
1. **Full import not tested** - The bootstrap process with full data takes considerable time and was not run to completion during implementation. Testing was done with unit tests and ArangoDB queries.

2. **Multi-refs not implemented** - The `add_sc_multi_refs()` function from the Python version is not implemented as per PRD scope.

3. **Per-language databases not implemented** - Only appdata.sqlite3 is used; per-language suttas_lang_{lang}.sqlite3 databases are not generated.

4. **Progress bars** - Not implemented for import progress (could be added with indicatif crate).

### Differences from Python Implementation
- Uses Rust async/await with tokio runtime for ArangoDB queries
- Uses custom `SuttaCentralData` struct to support both HTML and JSON content
- Error handling uses anyhow::Result instead of Python exceptions
- Logging uses tracing crate instead of Python logging

### Future Enhancements (Out of Scope)
- Add `add_sc_multi_refs()` for PTS references
- Generate per-language databases
- Add progress bars with indicatif
- Support additional languages beyond 'en' and 'pli'
- Caching of ArangoDB queries
- Incremental updates

### Testing Recommendations
To test the full import:
1. Ensure ArangoDB is running with suttacentral database
2. Ensure sc-data directory exists at bootstrap-assets-resources/sc-data
3. Run: `cd cli && cargo run --bin simsapa_cli bootstrap`
4. Monitor logs for successful import
5. Verify database records with SQL queries (see task 7.2)
6. Spot-check sample suttas (see task 7.3)

To test with limited dataset:
1. Set environment variable: `export BOOTSTRAP_LIMIT=100`
2. Run: `cd cli && cargo run --bin simsapa_cli bootstrap`
3. This will import only the first 100 records from each query
