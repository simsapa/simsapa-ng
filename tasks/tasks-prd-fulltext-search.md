...
- [x] 1.0 Set up Snowball stemmers module in backend
  - [x] 1.1 Create `backend/src/snowball/` directory structure: `mod.rs`, `among.rs`, `snowball_env.rs`, `algorithms/mod.rs`
  - [x] 1.2 Copy the Snowball Rust runtime files (`among.rs`, `snowball_env.rs`) from `pali-stemmer-in-snowball/assets/snowball/rust/src/snowball/` into `backend/src/snowball/`. Fix `use` paths to use `crate::snowball::` instead of `snowball::`.
  - [x] 1.3 Add a `compile-stemmers` Makefile target that iterates over all `.sbl` files in `pali-stemmer-in-snowball/assets/snowball/algorithms/` (excluding `dutch_porter.sbl`, `lovins.sbl`, `porter.sbl`) plus `pali-stemmer-in-snowball/algorithms/pali.sbl`, compiles each to Rust with `snowball {file}.sbl -rust -o backend/src/snowball/algorithms/{lang}`, and applies `sed -i 's/use snowball::/use crate::snowball::/g'` to each generated file.
  - [x] 1.4 Run `make compile-stemmers` to generate all 32 stemmer `.rs` files into `backend/src/snowball/algorithms/`.
  - [x] 1.5 Create `backend/src/snowball/algorithms/mod.rs` with `pub mod` declarations for all generated stemmer modules.
  - [x] 1.6 Create `backend/src/snowball/mod.rs` with: `Algorithm` enum (all 32 languages + fallback), `Stemmer` struct with `new(algo) -> Self` and `stem(word) -> Cow<str>` methods, and `lang_to_algorithm(lang_code: &str) -> Algorithm` mapping function (per PRD section 4.1 table). Reference `pali-stemmer-in-snowball/assets/rust-stemmers-simsapa/src/lib.rs` for the pattern.
  - [x] 1.7 Register the `snowball` module in `backend/src/lib.rs` (or `backend/src/main.rs` / module root).
  - [x] 1.8 Write unit tests in `backend/tests/test_snowball.rs`:
  ...
  - [x] 1.9 Run `cd backend && cargo test test_snowball` and verify all tests pass. Run `make build -B` to verify full compilation.

- [x] 2.0 Add Tantivy dependency and create search module with tokenizers and schemas
  - [x] 2.1 Add `tantivy = "0.25"` to `backend/Cargo.toml` dependencies.
  - [x] 2.2 Create `backend/src/search/` directory with `mod.rs`.
  - [x] 2.3 Implement `backend/src/search/tokenizer.rs`:
  ...
  - [x] 2.4 Implement `backend/src/search/schema.rs`:
  ...
  - [x] 2.5 Register the `search` module in `backend/src/lib.rs`.
  - [x] 2.6 Run `cd backend && cargo test` and `make build -B` to verify compilation with the new tantivy dependency.

- [x] 3.0 Implement index building (indexer) and index storage paths
  - [x] 3.1 Extend `AppGlobalPaths` in `backend/src/lib.rs` with three new fields: `index_dir: PathBuf` (`{app_assets_dir}/index/`), `suttas_index_dir: PathBuf` (`{index_dir}/suttas/`), `dict_words_index_dir: PathBuf` (`{index_dir}/dict_words/`). Initialize them in the constructor.
  - [x] 3.2 Implement `backend/src/search/indexer.rs`:
    ...
    - `build_sutta_index(db: &DatabaseHandle, index_dir: &Path, lang: &str) -> Result<()>` — queries suttas for the given language from the database, builds content by prepending `sutta_ref`, `title`, `title_pali` to `content_plain`, skips entries with no content, writes documents to the index using a 50MB writer. Ensures `IndexWriter` is committed and dropped before returning.
    - `build_dict_index(db: &DatabaseHandle, index_dir: &Path, lang: &str) -> Result<()>` — queries dict_words for the given language, builds content by prepending `word` and `synonyms` to `definition_plain`, skips entries with no definition, writes documents.
    - `build_all_indexes(appdata_db: &DatabaseHandle, dict_db: &DatabaseHandle, paths: &AppGlobalPaths) -> Result<()>` — queries distinct languages from both suttas and dict_words tables, calls `build_sutta_index` and `build_dict_index` for each.
    - `get_sutta_languages(db: &DatabaseHandle) -> Result<Vec<String>>` and `get_dict_word_languages(db: &DatabaseHandle) -> Result<Vec<String>>` — `SELECT DISTINCT language` queries.
  - [x] 3.3 Create an `INDEX_VERSION` constant (e.g., `"1.0"`) and a function `write_version_file(index_dir: &Path)` that writes a `VERSION` file containing the index schema version and stemmer version. Add a corresponding `read_version_file()` and `is_index_current()` check.
  - [x] 3.4 Write integration tests in `backend/tests/test_search.rs`: create a temporary directory, build a small sutta index with test data, verify the index directory and Tantivy files are created, verify the VERSION file is written. Similarly test dict_word indexing.
  - [x] 3.5 Run `cd backend && cargo test test_search` and verify tests pass.

- [x] 4.0 Implement fulltext search (searcher) and integrate with query_task
  - [x] 4.1 Implement `backend/src/search/searcher.rs`:
  ...
  - [x] 4.2 Wire `FulltextSearcher` into the application lifecycle: initialize it (lazily or at startup) and make it accessible from `SearchQueryTask`. Stored in `FULLTEXT_SEARCHER` OnceLock with `init_fulltext_searcher()` and `try_get_fulltext_searcher()`.
  - [x] 4.3 In `backend/src/query_task.rs`, implement the `SearchMode::FulltextMatch` branch in `results_page()`:
    - Call `FulltextSearcher::search_suttas()` or `search_dict_words()` based on `SearchArea`.
    - The existing `SearchResult` struct in `types.rs` already has all required fields: `uid`, `title`, `source_uid`, `snippet`, `score`, `lang`, `sutta_ref`, `nikaya`.
    - Results populated directly from Tantivy documents into `SearchResult` structs.
    - `highlighted_result_pages` and `db_query_hits_count` populated for pagination.
  - [x] 4.4 Add search tests: index test data, search with stemmed terms (e.g., search "bhikkhu" matches "bhikkhūnaṁ"), verify dual-field boost ranking, verify filter narrowing, verify snippet HTML uses `<span class='match'>`.
  - [x] 4.5 Run `cd backend && cargo test` and `make build -B` to verify everything compiles and tests pass.

- [x] 5.0 Add CLI commands for index management
  - [x] 5.1 Add `index` subcommand group to the CLI (`cli/`) with the following subcommands:
  ...
  - [x] 5.2 Implement the CLI handlers that call the backend `search::indexer` functions (`build_all_indexes`, `build_sutta_index`, `build_dict_index`). Add progress logging with `tracing`.
  - [x] 5.3 Test CLI commands manually: `cargo run -p cli -- index build`, verify index directories are created under the configured `index_dir`.

- [x] 6.0 Integrate index building into bootstrap pipeline
  - [x] 6.1 In `cli/src/bootstrap/mod.rs`, after the appdata archive creation (step 3 in the current pipeline), add a call to build fulltext indexes for base languages (en, pli, san) and all dict_words languages.
  - [x] 6.2 After building base indexes, create `index.tar.bz2` from the `index/` directory and move it to the release directory. Ensure the `IndexWriter` is dropped/committed before tar runs.
  - [x] 6.3 In the per-language loop (step 5), after importing each language's suttas into its `.sqlite3`, call `build_sutta_index()` for that language. Modify `create_database_archive()` (or create a new function) to include both the `.sqlite3` file and `index/suttas/{lang}/` in the per-language tarball.
