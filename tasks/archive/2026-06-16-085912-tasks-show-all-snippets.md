...
- `backend/src/search/searcher.rs` - `render_snippet` (shared by all four doc-builders `sutta_/dict_/library_/bold_definition_doc_to_result`; refactor to range-based non-nested highlight using `Snippet::fragment()` + `Snippet::highlighted()` ∪ literal ranges — this also fixes Dictionary fulltext nesting); `search_single_index` now returns `(score, DocAddress, SearchResult)`; `search_indexes` carries `(score, lang_key, DocAddress, SearchResult)`, slices at the record level, then (flag on, Suttas/Library) calls `expand_doc_occurrences`; `enumerate_match_ranges` (re-tokenizes stored content with the `{lang}_stem` analyzer, keeps tokens whose stem ∈ query stems); `expand_doc_occurrences` (windows each occurrence via `SearchQueryTask::fragment_around_offset`, focal-highlights via `wrap_ranges`, `is_snippet: true`, zero-occurrence fallback to `base`); `#[cfg(test)] mod tests` (`test_show_all_snippets_expands_per_occurrence`).
- `backend/src/search/types.rs` - `SearchFilters` gains `show_all_snippets: bool` (`#[serde(default)]`, default `false`); only Suttas/Library honour it.
...
- `assets/qml/tst_FulltextResultsFindQuery.qml` - **new** QML TestCase for `FulltextResults.derive_find_query()` (matched word + following words; empty when no match span; tag/punctuation/ellipsis handling). Auto-discovered by `make qml-test`.
- `src-ts/find.ts` / `src-ts/find.test.ts` - `FindManager.makeInterWordFlexible()` makes inter-word gaps in the search term match whitespace **and** punctuation (bridges punctuation-stripped `content_plain` vs. punctuation-bearing rendered HTML); applied in `performSearch` after accent folding. Uses an explicit class (not `\W`, which would over-match Pāli accented letters; not the `u` flag, which would strict-parse the whole pattern). Tests cover `non-reactive` matching and the helper.
- `backend/tests/test_show_all_snippets_acceptance.rs` - **new** integration acceptance tests against the real appdata DB: `cnd8/pli/ms` fulltext `pajahati` expands into focal-highlighted non-nested per-occurrence snippets (incl. distinct `pajahati` / `pajahitvā` snippets, focal-only), flag-off returns one whole-record row, and the `snippet_exclude` filter drops every snippet whose window contains the term (diacritic-insensitive) while leaving the record present and the record count unchanged. Record isolated via `uid_prefix`.
- `backend/tests/test_contains_multi_snippet.rs` - **new** integration test: Contains `show_all_snippets` expands a record into one focal-highlighted, non-nested, `is_snippet` row per literal occurrence; record total unchanged; literal-only (never highlights inflections).
- `backend/tests/test_highlight_pipeline.rs` - **new** integration test: Fulltext + Contains suttas snippets are highlighted and **non-nested** end-to-end (producer + `highlight_row` fallback); Contains is literal-only (never highlights inflections).
- `backend/tests/helpers/mod.rs` - `app_data_setup()` now also calls `init_fulltext_searcher()` (the real app opens the Tantivy indexes lazily via QML, which tests have no trigger for), fixing the pre-existing "Fulltext searcher not initialized" failures that made all FulltextMatch integration tests return empty.
...
- [x] 2.0 Refactor the snippet-highlight pipeline to be producer-owned, range-based, and non-nested (fixes the existing double-highlight bug and prepares for focal snippet highlighting)
  - [x] 2.1 Add a shared highlight module (`backend/src/highlight.rs`, registered in `lib.rs`) with: `merge_ranges(ranges) -> Vec<Range<usize>>` (sort + coalesce overlapping/adjacent), `wrap_ranges(text, &[Range]) -> String` (one `<span class='match'>` per merged range — non-nested), `literal_ranges(text, term) -> Vec<Range>` (all normalized, case-insensitive occurrences), and a focal single-occurrence helper (range around a known byte offset). All operate on char-boundary-safe byte ranges.
  - [x] 2.1a Add an **offset-based windowing helper** alongside `fragment_around_text` (`query_task.rs:183`), e.g. `fragment_around_offset(content, match_start, match_len, chars_before, chars_after) -> (window_text, focal_range_in_window)`. Reason: `fragment_around_text` only windows around the **first** occurrence (`content.to_lowercase().find(term)`), but multi-snippet needs a window around the **Nth** occurrence at a known offset, plus the focal match's range *within that window* (to pass to `wrap_ranges`). Reused by Tasks 3 (Fulltext) and 4 (Contains). Keep the existing `fragment_around_text` for the single-snippet fast path.
  - [x] 2.2 Refactor `render_snippet` (`searcher.rs:836`) to range-based: take `Snippet::fragment()` + `Snippet::highlighted()` (stemmed ranges), union with `literal_ranges(fragment, normalize_plain_text(query))`, `merge_ranges`, then `wrap_ranges`. Pass the query text into `render_snippet` (signature change). **`render_snippet` is shared by all four doc-builders** — `sutta_doc_to_result`, `dict_doc_to_result`, `library_doc_to_result`, `bold_definition_doc_to_result` (`searcher.rs:691/723/765/797`) — so update all four call sites; the refactor also fixes the same nested-span bug on the **Dictionary** fulltext path (out of scope for multi-snippet, but it shares `render_snippet`). Confirm dict/bold/library snippets still render correctly. Output is non-nested and covers stemmed + literal (Req 12d).
  - [x] 2.3 Move ContainsMatch highlighting into the producers: in `db_sutta_to_result` and `db_book_spine_item_to_result`, highlight the plain fragment via `wrap_ranges(fragment, literal_ranges(fragment, normalize_plain_text(query)))` (all occurrences) so the snippet arrives already highlighted (Req 12c).
  - [x] 2.4 Convert the central pass into a fallback: in `highlight_row` (`query_task.rs:2361`), highlight via `highlight_query_in_content` **only when** the snippet does not already contain `class='match'` (producer left it plain — e.g. TitleMatch/UidMatch/non-DPD dict). Already-highlighted snippets (Fulltext, Contains, and later the focal all-snippets rows) pass through unchanged. This removes the nested-span bug and means **no separate `is_snippet` highlight guard is needed** later.
  - [x] 2.5 Regression tests (write now, run at 2.7):
  ...
  - [x] 2.6 Manually confirm no visible regression in normal search highlighting (single-snippet Fulltext + Contains still show highlighted matches) — agent verifies via tests + compile; GUI check is the user's.
  - [x] 2.7 Build (`make build -B`) and run `cd backend && cargo test` for the new highlight tests; confirm existing highlight-related tests still pass.
...
- [x] 4.0 Contains Match (SQLite FTS5): expand each page record into one focal-highlighted `SearchResult` per literal occurrence, for Suttas and Library
  - [x] 4.1 Add a `QueryTask` helper mapping one `Sutta` row → `Vec<SearchResult>` (one per literal occurrence of the normalized query in the same content used for the snippet, `content_plain`; window each via `fragment_around_offset` (2.1a); focal-highlight via the 2.1 helper; `is_snippet: true`; shared record metadata). Use the same normalization as the highlight/snippet path so occurrence offsets line up. **Zero-occurrence fallback:** if no literal occurrence is found, emit one `db_sutta_to_result` snippet so the record still appears. Falls back to single-snippet `db_sutta_to_result` (`is_snippet: false`) when the flag is off.
  - [x] 4.2 In `suttas_contains_match_fts5`, swap the `.map(|s| db_sutta_to_result(s))` collect for the flag-aware expansion (`flat_map` when on); keep `total` = record `COUNT(*)`.
  - [x] 4.3 Add the equivalent expansion in `book_spine_items_contains_match_fts5` (book content + `page_number`), same flag.
  - [x] 4.4 Verify Dictionary contains-match paths are untouched (out of scope, single-snippet).
  - [x] 4.5 Tests (write now, run at 4.6): a row with two literal occurrences expands to two `SearchResult`s, each focal-highlighted (single non-nested span), `is_snippet: true`; `pajahitvā` is **not** highlighted for query `pajahati`. Flag-off path returns one row, highlighted by the fallback.
  - [x] 4.6 Build (`make build -B`) + `cargo test`; sanity-check a Suttas contains query expands a multi-occurrence record.
...
- [x] 6.0 Advanced Search Options UI + header dedup
...
- [x] 7.0 Snippet-aware find-bar jump
  - [x] 7.1 In `update_page()`, derive `find_query` per row from the snippet HTML (as above), append to `results_model`, add the `find_query` role to the delegate's `required property` set. (Added as a model role consumed via `current_result_data()`, mirroring the `is_snippet` role — not rendered in the delegate, so no `required property` declaration is needed.)
  - [x] 7.2 Thread `find_query` into the open path: it rides along in `current_result_data()`; add a `find_query` key in `new_tab_data()` (~313) so it reaches the find-on-open block. Trace each hop from click → `show_result_in_html_view()` and confirm it survives. (Click → `update_item()` → `current_result_data()` (model row) → `show_result_in_html_view(result_data)` → `new_tab_data(result_data)` → `tab_data.find_query`.)
  - [x] 7.3 In the find-on-open block (`:1019–1027`), set `root.pending_find_query` to the result's `find_query` when present, else fall back to `root.last_query_text`; keep all existing gates unchanged.
  - [x] 7.4 Factor the `find_query` derivation into a small pure JS function so it is testable; add a check: `… pajahati na upādiyati *pajahitvā* ṭhito …` → `pajahitvā ṭhito`; no match span → `""`. (`FulltextResults.derive_find_query()`; `assets/qml/tst_FulltextResultsFindQuery.qml`.)
  - [x] 7.5 Confirm unchanged behaviour when `open_find_in_sutta_results` is off and for non-sutta results; a single-snippet result still jumps to its one snippet's matched word + following words. (Gates unchanged; non-sutta rows take the `else` branch and never set `pending_find_query`; single-snippet rows derive `find_query` from their one snippet and fall back to `last_query_text` when there is no match span.)
  - [x] 7.6 Build (`make build -B`); GUI verification of the jump is the user's.
...
- [x] 8.0 Integration acceptance, full test run, and documentation
  - [x] 8.1 Add the `cnd8/pli/ms` acceptance test against the real appdata DB (fulltext `pajahati`, `show_all_snippets` on): the record expands into per-occurrence focal-highlighted, non-nested snippets (each a single span), including distinct snippets focally highlighting `pajahati` and `pajahitvā`; flag-off returns one whole-record row. (The PRD's "exactly two" used a truncated excerpt; the real record yields 4 occurrences. The record is isolated with a `uid_prefix` filter. `backend/tests/test_show_all_snippets_acceptance.rs`.)
  - [x] 8.2 Add an integration check combining `show_all_snippets` + `snippet_exclude` (exclude `pajahitvā` → every snippet whose **window** contains it is dropped; the `pajaheyyuṁ` snippet survives so the record stays; diacritic-insensitive `pajahitva` matches the same set; record count unchanged).
  - [x] 8.3 Run `cd backend && cargo test` and confirm green (ignore pre-existing unrelated failures per project guidance). (All green except the pre-existing, unrelated `test_dpd_lookup_generate_json` — a DPD `suggati` gloss-ordering mismatch with nothing to do with snippets.)
  - [x] 8.4 Documented in `docs/search-snippet-highlight-pipeline.md` (the consolidated doc used throughout this feature instead of a separate `show-all-snippets.md`): the refactored highlight pipeline (producer-owned, non-nested, per-mode semantics), fulltext stemmed vs contains literal enumeration, focal-only vs all-occurrence highlighting, exclusion semantics, header dedup, snippet-aware find-bar jump (incl. same-sutta immediate re-run + `find.ts` punctuation tolerance), session-only state, cache reuse.
  - [x] 8.5 Added a pointer in `PROJECT_MAP.md` (Search & Lookup) and the notable-feature-docs list in `AGENTS.md` (the real target of the `CLAUDE.md` symlink) references `docs/search-snippet-highlight-pipeline.md`, now extended with the find-bar jump.
