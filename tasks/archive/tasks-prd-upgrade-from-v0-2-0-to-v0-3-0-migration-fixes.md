...
- [x] 6.0 Post-implementation fixes from PRD §11 (dialog dedup, processing UI, marker I/O, orphan recovery, export orphan tolerance, bypass logging)
  ...
  - [x] 6.9 Remove stale `chanting-recordings/` directory during upgrade (PRD §11.7): extend `check_delete_files_for_upgrade()` in `backend/src/lib.rs` so that, after the `index/` deletion, it also removes the `chanting-recordings/` directory (resolved via `get_chanting_recordings_dir()`), gated on the same `try_exists() == Ok(true)` check and using `fs::remove_dir_all`. Update the function's doc comment to list `chanting-recordings/` alongside the other removed paths. Verified with `make build -B`.
