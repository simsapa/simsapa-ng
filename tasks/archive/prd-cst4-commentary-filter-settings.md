# PRD: CST & Commentary Filter Settings

## Introduction/Overview

Users need control over whether CST (Chaṭṭha Saṅgāyana Tipiṭaka 4) Pāli texts and commentary texts (Aṭṭhakathā, Ṭīkā) appear in search results and translation tabs. Currently, all related texts are loaded unconditionally when viewing translations, and search result filtering for CST/commentary sources is not exposed as a user setting.
...
- Whether CST mūla (root) texts appear in search results
- Whether commentary texts appear in search results
- Whether CST mūla texts appear in translation tabs

## Goals

1. Give users fine-grained control over CST and commentary visibility in search results and translation tabs.
2. Reduce noise from duplicate Pāli results (CST vs MS) while preserving access when needed.
...
- As a Pāli student, I want to include CST texts in my search results so that I can compare variant spellings between MS and CST editions.
- As a reader, I want to hide CST duplicates from search results (default) so that my results are less cluttered.
...
| 2 | Include cst mūla in search results | `include_cst_mula_in_search_results` | `false` | Search results |
| 3 | Include commentary in search results | `include_commentary_in_search_results` | `true` | Search results |
| 4 | Include cst mūla in translations | `include_cst_mula_in_translations` | `false` | Translation tabs |
...
- Include cst mūla in search results
- Include commentary in search results
- Include cst mūla in translations

### FR-3: Search Result Filtering — CST Mūla

7. When `include_cst_mula_in_search_results` is `false` (default), exclude sutta records where the UID ends with `/cst` AND the reference part (before the first `/`) does NOT contain `.att`, `.tik`, or `.xml`.
   - Excluded: `mn1/pli/cst`, `dn22/pli/cst`
   - NOT excluded: `mn1.att/pli/cst` (commentary), `mn1.tik/pli/cst` (sub-commentary), `s0502.att.xml/pli/cst` (XML-based record)
8. When `include_cst_mula_in_search_results` is `true`, no CST filtering is applied.
...
10. When `include_commentary_in_search_results` is `false`, exclude sutta records where the reference part (before the first `/`) contains `.att` or `.tik` (but NOT `.xml` suffixed records like `s0502.att.xml`).
    - Excluded: `mn1.att/pli/cst`, `mn1.tik/pli/cst`
    - NOT excluded: `s0502.att.xml/pli/cst`
...
### FR-6: Translation Tab Filtering — CST Mūla

15. In `get_translations_data_json_for_sutta_uid()`, when `include_cst_mula_in_translations` is `false` (default), exclude records where UID ends with `/cst` AND the reference part does NOT contain `.att`, `.tik`, or `.xml`.
    - This uses the same logic as FR-3 but applied to the translations query.
16. When `include_cst_mula_in_translations` is `true`, no CST filtering is applied to translations.

### FR-7: Translation Tab Sort Order

17. When CST translations are included, `mn1/pli/cst` should appear right after `mn1/pli/ms`, grouped with other Pāli records. Commentaries (when enabled) follow in the same Pāli group. The existing `sort_suttas()` function should handle this — verify and adjust if needed.
...
| `mn1/pli/cst` | Only if `include_cst_mula_in_translations` is `true` |
| `mn1.att/pli/cst` | Only if `include_commentary_in_translations` is `true` (NOT affected by cst mūla filter) |
| `mn1.tik/pli/cst` | Only if `include_commentary_in_translations` is `true` (NOT affected by cst mūla filter) |
...
- Filtering XML-based records (e.g., `s0502.att.xml/pli/cst`) — these are always included regardless of cst/commentary filters.
...
| `backend/src/query_task.rs` | Add cst mūla and commentary filtering logic in search modes that already have source filtering |
| `backend/src/db/appdata.rs` | Modify `get_translations_data_json_for_sutta_uid()` to conditionally include commentary and cst |

### Filtering Logic

To distinguish mūla cst records from commentary/XML cst records, the filter should check:
```
UID ends with "/cst" AND reference_part NOT LIKE "%.att%" AND reference_part NOT LIKE "%.tik%" AND reference_part NOT LIKE "%.xml%"
```

In SQL terms, for excluding cst mūla:
```sql
NOT (uid LIKE '%/cst' AND uid NOT LIKE '%.att%/cst' AND uid NOT LIKE '%.tik%/cst' AND uid NOT LIKE '%.xml%/cst')
```
...
- Implement cst mūla filtering in `query_task.rs` search modes
...
- Implement conditional cst mūla inclusion
...
- Enabling "include cst mūla in search results" adds CST root texts to results without affecting commentary or XML records.
- Enabling "include commentary in translations" shows `.att` and `.tik` tabs.
- Enabling "include cst mūla in translations" shows `*/pli/cst` tab after `*/pli/ms`.
...
2. Are there edge cases where a sutta exists in MS but not in CST, or vice versa, that could affect the expected tab behavior?
3. Does the current `sort_suttas()` function already sort by source in a way that places `cst` after `ms`, or does it need adjustment?
