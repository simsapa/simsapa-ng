...
- [x] 5.0 Implement the **Windows** backend of `GlobalHotkeyManager` (`RegisterHotKey` + `SendInput` re-emit + transient unregister/register around the synthesized event, mirroring Goldendict-ng's proven approach).
  - [x] 5.1 Create `cpp/global_hotkey_win.cpp` (compiled only on `Q_OS_WIN`). Implement `init()` to obtain an `HWND` (use a hidden helper window or the main window's `winId()`). *(Uses `RegisterHotKey(NULL, ...)` instead of an HWND: with a NULL window handle, WM_HOTKEY is posted to the calling thread's message queue, which Qt picks up via the installed native event filter. Avoids needing to keep a hidden helper QWidget alive.)*
  - [x] 5.2 Implement `registerHotkey()` to call `RegisterHotKey(hwnd, id, modifier, vk)` for the first chord; persist the second key separately in the `HotkeyEntry` for the `state2` matcher. *(Also calls `RegisterHotKey` for the second chord with `id+1` when `key2 != 0 && key2 != key1`, matching Goldendict-ng. For `Ctrl+C+C` where `key2 == key1`, only one registration is needed — both presses fire the same WM_HOTKEY.)*
  - [x] 5.3 Install a Windows native event filter (`QAbstractNativeEventFilter::nativeEventFilter`) to intercept `WM_HOTKEY` messages and feed them into the shared `state2` matcher. *(`WinHotkeyEventFilter` in an anonymous namespace, installed on `QCoreApplication::instance()` from `init()` and removed in `shutdown()`.)*
  - [x] 5.4 In the matcher, when a hotkey first chord fires and the user's intent is to copy (i.e. it's `Ctrl+C` or any sequence whose first chord is a copy combo), re-emit the keystroke via `SendInput` so the foreground application performs its copy — temporarily `UnregisterHotKey`/`RegisterHotKey` around the `SendInput` call to avoid retriggering ourselves. *(Implemented in `checkStateWin()` via `reEmitKeystroke()`. Re-emits when `hs.key2 != 0` (first chord of a double-tap) OR when the chord itself is `Ctrl+C` / `Ctrl+Insert`.)*
  - [x] 5.5 Track the modifier-key state to avoid emitting modifier presses that are already physically held (port the `GetAsyncKeyState` checks from Goldendict-ng's `winhotkeywrapper.cc`). *(Done in `reEmitKeystroke()`; only down/up modifier events for modifiers whose physical state is unset are appended to the `INPUT` array.)*
  - [x] 5.6 Implement `unregister()` to `UnregisterHotKey` everything and reset the id counter. *(`unregisterAll()` walks `m_hotkeys`, calling `UnregisterHotKey` for each `id` (and `id+1` when a distinct second chord was registered), then clears the list. `m_winNextId` rolls over at `0xBFFF` to avoid the GlobalAddAtom-reserved range.)*
  - [x] 5.7 Add `cpp/global_hotkey_win.cpp` to `app_files` in `CMakeLists.txt` with appropriate `if(WIN32)` guards.
  - [x] 5.8 Run `make build -B` (Linux build remains green; the Windows path is compile-checked locally via the `#ifdef` guards and will be exercised on a Windows build by the user). *(Linux build clean; Windows path is `#ifdef Q_OS_WIN` guarded and ready for a Windows build.)*

- [x] 6.0 Implement the **macOS** backend of `GlobalHotkeyManager` (Carbon `RegisterEventHotKey`, AX selected-text via `AXUIElementCopyAttributeValue`/`kAXSelectedTextAttribute` with synthesized `⌘C` fallback wrapped in `suspendHotkeys()`/`resumeHotkeys()`, and the first-use Accessibility permission dialog that opens System Preferences).
  - [x] 6.1 Create `cpp/global_hotkey_mac.mm` (compiled only on `Q_OS_MACOS`). Install an `EventHotKey` handler via `InstallApplicationEventHandler` listening for `kEventClassKeyboard` / `kEventHotKeyPressed`. *(Static `hotKeyHandler` callback dispatches to `GlobalHotkeyManager::checkStateMac(int)` via the `userData` pointer.)*
  - [x] 6.2 Build a Qt-key → native macOS keycode map at startup using `TISCopyCurrentKeyboardLayoutInputSource` + `UCKeyTranslate` (port the `MacKeyMapping` helper from Goldendict-ng with a fresh implementation). *(Lazy-built `g_keyMap` filled by `buildKeyMap()`; `qtKeyToMacKey()` falls back to a static virtual-key table for non-printable keys (Esc, Tab, F1-F15, arrows, etc.).)*
  - [x] 6.3 Implement `registerHotkey()` to call `RegisterEventHotKey()` for the first chord (and the second chord with `id+1` to support double-tap matching). *(`m_macNextId` advances by 2; second chord is registered only when `key2 != 0 && key2 != key1`. Qt::CTRL maps to `cmdKey` on macOS so the cross-platform default `Ctrl+C+C` becomes `Cmd+C+C`.)*
  - [x] 6.4 In the event handler, route the activated id into the shared `state2` matcher. *(`checkStateMac(int)` runs the same `state2` flow as the Windows backend, including the `id+1` / repeated-key second-chord logic.)*
  - [x] 6.5 Implement `getSelectedTextViaAXAPI()` using `AXUIElementCreateSystemWide()` + `AXUIElementCopyAttributeValue(focused, kAXSelectedTextAttribute)`. Return an empty `QString` if anything fails. *(File-scope helper in the anonymous namespace; safely releases all `CFTypeRef`s.)*
  - [x] 6.6 Implement `sendCmdC()` using `CGEventCreateKeyboardEvent` + `CGEventPost(kCGAnnotatedSessionEventTap, …)` for both keyDown and keyUp, with `kCGEventFlagMaskCommand` set.
  - [x] 6.7 Implement `suspendHotkeys()` / `resumeHotkeys()` that `UnregisterEventHotKey` and re-register every entry, used to bracket the synthesized `⌘C`. *(`suspendHotkeysMac()` / `resumeHotkeysMac()`; `resumeHotkeysMac()` reuses the persisted `id` so the existing `state2` matcher keeps working.)*
  - [x] 6.8 On hotkey activation: call `getSelectedTextViaAXAPI()` first; if empty, `suspendHotkeys()` → `sendCmdC()` → `resumeHotkeys()`, then read `QGuiApplication::clipboard()->text()` after a short delay. *(AX path writes directly to `QGuiApplication::clipboard()` so the shared post-activation pipeline (task 7) reads it the same way on every platform; no extra delay needed in that case. The `⌘C` fallback relies on the existing 80 ms delay in the activation pipeline.)*
  - [x] 6.9 Implement `checkAndRequestAccessibilityPermission()`: on first activation, if `AXIsProcessTrusted()` returns false, show a Qt message box explaining the requirement with a button that opens `x-apple.systempreferences:com.apple.preference.security?Privacy_Accessibility` via `[NSWorkspace openURL:]`. *(Gated by `m_macAxPromptShown` so the dialog only appears once per session.)*
  - [x] 6.10 Add `cpp/global_hotkey_mac.mm` to `app_files` in `CMakeLists.txt` with `if(APPLE)` guards; link `Carbon`, `AppKit`, `ApplicationServices` frameworks. *(Added inside the existing `if(APPLE AND NOT IOS)` blocks for both source listing and `target_link_libraries`.)*
  - [x] 6.11 Run `make build -B` to confirm the cross-platform skeleton still compiles on Linux (the macOS path is `#ifdef`-guarded). *(Clean build; macOS path will be exercised on a Mac build by the user.)*
