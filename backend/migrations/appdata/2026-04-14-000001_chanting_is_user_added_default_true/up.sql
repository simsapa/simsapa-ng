-- Flip the DEFAULT on is_user_added from 0 to 1 for chanting tables, so that
-- runtime inserts (e.g. from the UI) are marked user-added without having to
-- pass the column explicitly. Bootstrap code must always set is_user_added
-- explicitly (to 0 for seeded rows). SQLite doesn't support ALTER COLUMN
-- DEFAULT, so we rebuild each table.
