...
fn test_epub_custom_title_author_override() {
    ...
    let result = app_data.import_epub_to_db(&epub_path, book_uid, custom_title, custom_author, None, None, true);
...
fn test_epub_extracted_metadata_when_custom_empty() {
    ...
    let result = app_data.import_epub_to_db(&epub_path, book_uid, None, None, None, None, true);
    assert!(result.is_ok(), "EPUB import failed: {:?}", result.err());
...
fn test_pdf_custom_title_author_override() {
    ...
    let result = app_data.import_pdf_to_db(&pdf_path, book_uid, custom_title, custom_author, None, None, true);
